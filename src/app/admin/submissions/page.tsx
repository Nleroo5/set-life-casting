          ...
          .map((sub: any) => {
            ...
            return {
              ...
              profileData: {
                ...
                adminTag: sub.profiles.admin_tag as "green" | "yellow" | "red" | null,
                adminNotes: sub.profiles.admin_notes || null,
              },
            } as Submission;
          });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {selectedProjectId === "all" ? (
              ...
              filteredSubmissions.length === 0 ? (
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {filteredSubmissions.map((submission) => {
                    // Get headshot photo (or first photo)
                    const photoArray = submission.profileData.photos?.photos;
                    ...
                    if (submission.profileData.adminTag === "green") {
                    ...
                    } else if (submission.profileData.adminTag === "yellow") {
                    ...
                    } else if (submission.profileData.adminTag === "red") {
                    ...
                    return (
                      <div
                        key={submission.id}
                        className={`bg-linear-to-br from-white to-purple-50/30 rounded-xl border-2 ${borderClass} ${shadowClass} ${hoverBorderClass} ${hoverShadowClass} transition-all duration-300 overflow-hidden group relative cursor-pointer ${
                          selectedSubmission?.id === submission.id ? "ring-2 ring-accent ring-offset-2" : ""
                        }`}
                      ...
                      >
                        ...
                        <div className="absolute top-3 left-3 z-10">
                          <input
                            ...
                            className="w-5 h-5 text-accent border-accent/30 rounded focus:ring-2 focus:ring-accent cursor-pointer bg-white shadow-md"
                          />
                        </div>
                        ...
                        <div className="p-4">
                          <h3
                            ...
                            style={{ fontFamily: "var(--font-galindo)" }}
                          >
                            {submission.profileData.basicInfo?.firstName || 'Unknown'} {submission.profileData.basicInfo?.lastName || ''}
                          </h3>

                          {/* Role and Project */}
                          <p className="text-xs text-accent font-medium mb-2" style={{ fontFamily: "var(--font-outfit)" }}>
                          ...
                          >
                            <p suppressHydrationWarning>
                              {submission.profileData.appearance?.gender || 'N/A'} {age !== null && `• ${age} years`}
                            </p>
                            ...
                            {(submission.profileData.appearance?.height || submission.profileData.appearance?.weight) && (
                              <p>
                                {submission.profileData.appearance?.height || 'N/A'} {submission.profileData.appearance?.weight && `• ${submission.profileData.appearance.weight} lbs`}
                              </p>
                            )}

                            {/* Admin Notes Preview */}
                            {submission.profileData.adminNotes && (
              ...
              // Show submissions grouped by role (when project is selected) - keep existing logic
              Object.entries(
                filteredSubmissions.reduce((acc, submission) => {
                  const roleId = submission.roleId;
                  if (!acc[roleId]) {
                    acc[roleId] = [];
                  }
                  acc[roleId].push(submission);
                  return acc;
                }, {} as Record<string, Submission[]>)
              ).map(([roleId, roleSubmissions]) => {
                ...
                return (
                  <div
                    key={roleId}
                    className="bg-linear-to-br from-white to-purple-50/30 rounded-xl p-6 border-2 border-accent/20"
                  >
                    {/* Role Header */}
                    <div
                    ...
                    >
                      <div>
                        ...
                        <p
                          ...
                          style={{ fontFamily: "var(--font-outfit)" }}
                        >
                          {roleSubmissions.length} submission{roleSubmissions.length !== 1 ? 's' : ''}
                        </p>
                      </div>
